...
<div class="container">
    ...
    <div style="margin-top:20px;">
        <?php if (empty($letters)): ?>
            <div class="empty-state"><div class="emoji">💌</div><p>Belum ada surat.</p></div>
        <?php else: ?>
            <?php foreach ($letters as $letter): ?>
                <div class="card" style="margin-bottom:15px;">
                    <h3><?php echo escape($letter['title']); ?></h3>
                    <p><?php echo nl2br(escape($letter['content'])); ?></p>
                    ...
                    <div style="margin-top:10px; display:flex; gap:10px;">
                        <button class="btn btn-secondary" onclick="openModal(`<?php echo escape('
                            ...
                            <form method="POST">
                                ...
                                <div class="form-group"><label>Judul</label><input type="text" name="title" value="' . escape($letter['title']) . '" required></div>
                                <div class="form-group"><label>Isi</label><textarea name="content" rows="6" required>' . escape($letter['content']) . '</textarea></div>
                                <button type="submit" class="btn">Update</button>
                            </form>
                        '); ?>`)">Edit</button>
                        <a href="/eternity/letters.php?delete=<?php echo $letter['id']; ?>" class="btn" style="background:#000;color:#fff;" onclick="return confirm('Yakin hapus?')">Hapus</a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
